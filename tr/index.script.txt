...
(() => {
  ...
  const commandActions = {
    ...
    developer: { type: "same-tab", target: "../developer.html" }
  };
  ...
  function runCommand(value) {
    ...
    const special = runSpecial(command);
    if (special) {
      emitLines(special);
      if (command === "flag" || command === "aliens" || command === "developer") performCommandAction(command);
      output.scrollTop = output.scrollHeight;
      return;
    }
    emitLines(commandMap[command] || ["Komut bulunamad?.", "Dene: help"]);
    output.scrollTop = output.scrollHeight;
